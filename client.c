...
int main(int argc, char* argv[]){
    ...
    phraseM temp;
    temp.taille = 3;
    temp.listeMots = malloc(sizeof(mot) * temp.taille);
    
    //while(1){
    if(recv(dS, &temp, sizeof(temp), 0)==-1){
        perror("reception du message impossible");
        exit(EXIT_FAILURE);
    }
    printf("%s",temp.listeMots[0].m );
    // printf("%s", temp.listeMots[1].m);
    // printf("%s", temp.listeMots[2].m);
